			...
			<form>
				<fieldset class="uk-fieldset uk-fieldset-alt md-bg-white sc-padding-medium">
					<div class="uk-child-width-1-2@m uk-grid" data-uk-grid>
						<div class="uk-child-width-1-2@m uk-grid">
							<div>
								<client-only>
									<Select2 v-model="selectedInvoiceType" :options="invoiceTypeList" @change="onInvoiceTypeChanged"
						...
						<div class="uk-child-width-1-2@m uk-margin-remove-top uk-grid">
							...
							<div class="sc-padding-small-top">
								<ScInput v-model="formData.receiptDate" :config="{ wrap: true, dateFormat: 'Y-m-d' }" v-flatpickr
									mode="outline">
									<label>Tarih</label>
								</ScInput>
							</div>
						...
						<div class="uk-child-width-1-2@m uk-grid">
							...
							<div>
								<client-only>
									<Select2 v-model="formData.inWarehouseId" :options="warehouseList"
				...
				<fieldset class="uk-fieldset uk-fieldset-alt md-bg-white sc-padding-medium sc-padding-remove-top">
					...
					<div class="uk-margin-medium uk-margin-remove-left">
						...
						<client-only>
							<Datatable id="sc-dt-item-receipt-details-table" ref="itemReceiptDetailsTable" :data="details"
				...
				<div class="uk-margin-large-top">
					...
					<button type="button" @click="onSubmit" v-show="hasViewAuth('Invoice', 1)"
						class="sc-button sc-button-primary sc-button-medium uk-margin-small-right">
					...
					<button type="button" @click="onDelete" v-show="hasViewAuth('Invoice', 2)"
...
import OpenReceiptList from '~/logical_components/OpenReceiptList.vue';
	...
	methods: {
		async bindModel() {
			...
			try {
				...
				const getData = (await api.get('Invoice/' + this.formData.id + '/'
					+ (this.selectedInvoiceType > 0 ? this.selectedInvoiceType : this.receiptCategory))).data;
				if (getData && getData.invoiceTypeList)
					this.invoiceTypeList = getData.invoiceTypeList.map((d) => {
						return {
							id: d.typeNo,
							text: d.typeText,
						};
					});
				else this.invoiceTypeList = [];
				...
				if (getData && getData.id > 0) {
					...
					this.selectedInvoiceType = getData.receiptType;
				...
				this.selectedInvoiceType = this.selectedInvoiceType ? this.selectedInvoiceType.toString() : null;

			} catch (error) {

			}
		},
		async onInvoiceTypeChanged(val) {
			this.formData.id = 0;
		...
		async onSubmit() {
			...
			try {
				...
				if (!forexOk) {
					this.showNotification('Fatura farklı döviz cinsleri içeremez.', false, 'error');
					this.isSaving = false;
					return;
				}
				...
				const api = useApi();
				const postResult = (await api.post('Invoice', this.formData)).data;
		...
		async onDelete() {
			...
			UIkit.modal.confirm('Bu irsaliyeyi silmek istediğinizden emin misiniz?').then(
				async function () {
					try {
						const api = useApi();
						const delResult = (await api.delete('Invoice/' + self.formData.id)).data;
						if (delResult.result) {
