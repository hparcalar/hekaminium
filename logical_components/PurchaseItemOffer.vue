                ...
                <fieldset class="uk-fieldset uk-fieldset-alt md-bg-white sc-padding-medium sc-padding-remove-top">
                    ...
                    <div class="uk-grid" style="margin-top:10px;">
                        ...
                        <div class="uk-width-3-5@m">
                            <div v-for="(firm, firmIndex) in selectedFirms" :key="firm">
                                <h6 class="blue-header">
                                    <button type="button" @click="expandToggleFirm(firm)" class="sc-button sc-button-default sc-button-small uk-margin-small-right">
											<span :data-uk-icon="'icon:'+ (getFirmExpanded(firm) ? 'minus' : 'plus')" class="uk-icon"></span>
										</button>
                                    {{ getFirmName(firm) }}
                                    <span v-for="(firmSum, sumIndex) in sumOfFirmDetails(firm, false)" :key="firmSum.forexId" class="firm-summary">
                                        <b>{{ firmSum.forexCode }}</b>: {{ new Intl.NumberFormat('tr-TR').format(firmSum.overallTotal) }}
                                    </span>
                                </h6>
                                <table v-show="getFirmExpanded(firm)" class="uk-table uk-table-striped uk-table-responsive uk-table-small" style="width:100%">
                                    <tr>
                                    ...
                                    <tr v-for="(item, itemIndex) in formData.details" :key="itemIndex" :class="{'selected-offer': getFirmPriceOfDetail(item, firm).isFirmSelected}">
                                        <td>
                                        ...
                                        <td>
                                            <ScInput v-model="item.lineNumber" :read-only="true" type="number"></ScInput>
                                        </td>
                                        <td>
                                            <ScInput v-model="item.itemExplanation" :read-only="offerIsOrdered"></ScInput>
                                        </td>
                                        <td>
                                            <client-only>
                                                <Select2
                                                    ...
                                                    :settings="{ 'width': '100%', 'placeholder': 'Döviz Cinsi', 'allowClear': true }"></Select2>
                                            </client-only>
                                        </td>
                                        <td>
                                            <ScInput v-model="getFirmPriceOfDetail(item, firm).forexRate" :read-only="offerIsOrdered" @change="calculateTotal(item, firm)" type="number"></ScInput>
                                        </td>
                                        <td>
                                            <ScInput v-model="getFirmPriceOfDetail(item, firm).unitPrice" :read-only="offerIsOrdered" @change="calculateTotal(item, firm)" type="number"></ScInput>
                                        </td>
                                        <td>
                                            <ScInput v-model="item.quantity" :read-only="offerIsOrdered" @change="calculateTotal(item, firm, true)" type="number"></ScInput>
                                        </td>
                                        <td>
                                            <ScInput v-model="getFirmPriceOfDetail(item, firm).overallForexTotal" :read-only="true" type="number"></ScInput>
                                        </td>
                                        <td><ScInput v-model="getFirmPriceOfDetail(item, firm).overallTotal" :read-only="true" type="number"></ScInput></td>
                                        <td>
                                            <PrettyCheck name="isActive"
                                                :disabled="offerIsOrdered"
                                                @change="onChangeFirmOfferSelection($event, item, firm)"
                                                v-model="getFirmPriceOfDetail(item, firm).isFirmSelected" class="p-icon">
                        ...
                        <div class="uk-width-2-5@m">
                            <h6>
                                Sipariş Özeti
                                <button v-show="!offerIsOrdered" type="button" @click="createItemOrder" style="float:right;"
								class="sc-button sc-button-primary sc-button-small uk-margin-small-right">
                            ...
                            <div v-for="(firm, firmIndex) in getChoosenFirms()" :key="firm.id">
                                <h6 class="blue-header">
                                    <button type="button" @click="expandForOrderToggleFirm(firm.id)" class="sc-button sc-button-default sc-button-small uk-margin-small-right">
                                        <span :data-uk-icon="'icon:'+ (getFirmForOrderExpanded(firm.id) ? 'minus' : 'plus')" class="uk-icon"></span>
                                    </button>
                                    {{ firm.text }}
                                    <span v-for="firmSum in sumOfFirmDetails(firm.id, true)" :key="firmSum.forexId" class="firm-summary">
                                        <b>{{ firmSum.forexCode }}</b>: {{ new Intl.NumberFormat('tr-TR').format(firmSum.overallTotal) }}
                                    </span>
                                    <button v-show="firmIsOrdered(firm.id)" type="button" @click="showItemOrder(firm.id)" style="float:right;"
                                    class="sc-button sc-button-primary sc-button-small uk-margin-small-right">
                                        <span data-uk-icon="icon: link" class="uk-icon"></span>Sipariş: {{ getFirmOrderNo(firm.id) }}
                                    </button>
                                </h6>
                                <table v-show="getFirmForOrderExpanded(firm.id)"
                                     class="uk-table uk-table-striped uk-table-responsive uk-table-small" style="width:100%">
                                    <tr>
                                        <th>Sıra</th>
                                        <th>Açıklama</th>
                                        <th>Döviz Cinsi</th>
                            ...
                            <p v-for="(orderSum, sumIndex) in sumOfOrder()" :key="orderSum.forexId" style="margin-top:2px;margin-bottom:2px;" class="firm-summary">
                                <b>{{ orderSum.forexCode }}</b>: {{ new Intl.NumberFormat('tr-TR').format(orderSum.overallTotal) }}
                            </p>
                        </div>
                    </div>
                </fieldset>
			...
			<div class="uk-modal-dialog uk-width-2-3" uk-overflow-auto>
				<div class="uk-modal-body">
                    <div class="uk-overflow-auto" v-if="currentDetail" style="max-height:500px;">
                        <div v-for="(demand, demandIndex) in currentDetail.demands" 
...
.selected-offer, .selected-offer td{
