					...
					<form>
						<fieldset class="uk-fieldset uk-fieldset-alt md-bg-white sc-padding-medium">
							<legend class="uk-legend">
								Döküman Tipi Bilgileri
							</legend>
							<div class="uk-child-width-1-2@m uk-grid" data-uk-grid>
								<div>
									<ScInput v-model="formData.categoryName">
						...
						<div class="uk-margin-large-top">
							<button type="button" @click="onSubmit" v-show="hasViewAuth('Document',1)"
                            ...
                            <button type="button" @click="onDelete" v-show="hasViewAuth('Document',2)"
...
export default {
	...
	methods: {
        async bindModel(){
            ...
            const api = useApi();
            try {
                const getData = (await api.get('Attachment/Category/' + this.formData.id)).data;   
		...
		async onSubmit(){
            try {
                const api = useApi();
                const postResult = (await api.post('Attachment/Category', this.formData)).data;
        ...
        onCancel(){
            this.$router.push('/attachment-category/list');
        },
        async onDelete(){
