				...
				<ScCardHeader separator>
					<div class="uk-flex-middle uk-grid-small uk-grid" data-uk-grid>
						<div class="uk-flex-1">
							<ScCardTitle>
								Döküman Tipleri
								<button type="button" @click="newRecord" v-show="hasViewAuth('Document',1)"
...
export default {
    ...
    data: () => {
		return {
			visualData: [],
			dtColumns: [
				{ data: "categoryName", title: "Kategori Adı", visible: true, },
			],
			dtDHeaders: [],
			dtDOptions: {
				select: true,
				...
				// 	localStorage.setItem( 'attachmentCategoryListTableView', JSON.stringify(data) )
				// },
				// stateLoadCallback (settings) {
				// 	const dtState = JSON.parse( localStorage.getItem( 'attachmentCategoryListTableView' ) );
				// 	return dtState;
				// },
				buttons: [
					// {
    ...
    async mounted (){
        const api = useApi();
        const rawData = (await api.get('Attachment/Category')).data;
    ...
    methods: {
        dtButtonsInitialized () {
            ...
            // const ls = JSON.parse( localStorage.getItem( 'attachmentCategoryListTableView' ) );
        ...
        clickDetail: function (e, dt, type, indexes){
			this.$router.push('/attachment-category?id=' + this.visualData[indexes[0]].id);
        },
        ...
        newRecord(){
            this.$router.push('/attachment-category');
        }
    }
}
</script>
