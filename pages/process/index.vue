					...
					<form>
						<fieldset class="uk-fieldset uk-fieldset-alt md-bg-white sc-padding-medium">
							...
							<div class="uk-child-width-1-2@m uk-grid" data-uk-grid>
								<div>
									<ScInput v-model="formData.processCode">
										<label>Proses Kodu</label>
									</ScInput>
								</div>
								<div>
									<ScInput v-model="formData.processName">
										<label>Proses Adı</label>
									</ScInput>
								</div>
                                <div>
									<ScInput v-model="formData.processOrder" :type="'number'">
...
export default {
	...
	methods: {
        async bindModel(){
            ...
            const api = useApi();
            try {
                const getResult = (await api.get('Process/' + this.formData.id)).data;   
		...
		async onSubmit(){
            try {
                const api = useApi();
                const postResult = (await api.post('Process', this.formData)).data;
