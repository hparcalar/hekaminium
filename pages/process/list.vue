...
export default {
    ...
    data: () => {
		return {
			dtDData: [],
			dtColumns: [
				{ data: "processCode", title: "Proses Kodu", visible: true, },
				{ data: "processName", title: "Proses Adı", visible: true, },
                { data: "processOrder", title: "Sırası", visible: true },
			],
			dtDHeaders: [],
			dtDOptions: {
                order: [[2, 'asc']],
    ...
    async mounted (){
        const api = useApi();
        const rawData = (await api.get('Process')).data;
