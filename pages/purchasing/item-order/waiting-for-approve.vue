					...
					<div class="uk-grid">
						<div class="uk-width-3-4@m">
							<div class="uk-flex-left uk-grid">
								<div class="uk-button-group sc-padding-remove-left uk-width-expand uk-margin-medium"
									style="height:34px;">
									<button @click="approveDetails" type="button"
										class="sc-button sc-button-success sc-button-small uk-width-1-3" style="height:34px;">
										<span data-uk-icon="icon: check" class="uk-margin-small-right uk-icon"></span>
										Onayla
									</button>
									<button @click="denyDetails" type="button"
										class="sc-button sc-button-danger sc-button-small uk-width-1-3" style="height:34px;">
							...
							<client-only>
								<Datatable id="sc-dt-purchase-orders-waiting-for-approve-table" ref="buttonsTable" :data="visualData"
									:options="dtDOptions" :customColumns="dtColumns" :buttons="true"
						...
						<div class="uk-width-1-4@m">
							...
							<div v-if="selectedOrderRow && selectedOrderRow.id > 0" class="uk-overflow-auto"
								style="max-height:250px;">
								<div v-for="(demand, demandIndex) in selectedOrderRow.demandConsumes" :key="demandIndex"
									...
									<div class="uk-width-4-5@m">
										...
										<p class="uk-padding-remove uk-margin-remove"><b>Proje: </b> {{ demand.projectName }}</p>
										<p class="uk-padding-remove uk-margin-remove"><b>Açıklama:</b> {{ demand.itemExplanation }}</p>
										<p class="uk-padding-remove uk-margin-remove"><b>Parça No:</b> {{ demand.partNo }}</p>
										<p class="uk-padding-remove uk-margin-remove"><b>Boyut:</b> {{ demand.partDimensions }}</p>
										<p class="uk-padding-remove uk-margin-remove"><b>Miktar:</b> {{ demand.demandQuantity }}</p>
									</div>
	...
	data: () => {
		return {
			...
			dtColumns: [
				...
				{ data: "firmName", title: "Firma", width: "20%", visible: true, },
