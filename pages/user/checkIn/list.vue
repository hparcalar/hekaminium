      ...
      <ScCard class="uk-margin-top">
        ...
        <ScCardBody>
          <div class="uk-flex" style="margin-bottom:15px;">
            <div class="">
              ...
              <div class="field col-12 md:col-4">
                <Calendar id="time24" v-model="startDate" :showTime="false" dateFormat="dd.mm.yy" @date-select="bindList" :showSeconds="false" placeholder="Time" />
              </div>
            </div>
            ...
            <div style="margin-left:20px;">
              ...
              <div class="field col-12 md:col-4">
                <Calendar id="time24" v-model="endDate" :showTime="false" dateFormat="dd.mm.yy" @date-select="bindList" :showSeconds="false" placeholder="Time" />
              </div>
            </div>
          ...
          <DataTable :value="visualData" responsiveLayout="scroll"
						...
						dataKey="id"
						:paginator="true" showGridlines :rows="13"
						selectionMode="single"
						sortField="processDate" :sortOrder="-1"
						>
						<template #empty>
							Hiç kayıt yok.
						</template>
						...
						<Column field="processDate" header="Giriş Tarihi" sortable :style="{width: '25%'}" :headerStyle="{width: '25%'}">		
  ...
  data: () => {
    return {
      ...
      dtColumns: [
        ...
        {
          data: "processDate", title: "Giriş Tarihi", visible: true,
          render: function (data, ev, row) {
            return row && row.processDate ? moment(row.processDate).format('YYYY.MM.DD HH:mm') : '';
          },
        },
        {
          data: "exitDate", title: "Çıkış Tarihi", visible: true,
          render: function (data, ev, row) {
            return row && row.exitDate ? moment(row.exitDate).format('YYYY.MM.DD HH:mm') : '';
          },
        },
        ...
        {
          data: "processType", title: "Uyarı", visible: true,
          render: function (data, ev, row) {
            return row && row.autoCheckOut == 0 ? "-" : "Auto Check Out";
          },
        },
      ],
      dtDHeaders: [],
