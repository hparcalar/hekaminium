        ...
        <ScCardBody>
          <div>
            ...
            <div class="field col-12 md:col-4">
              <Calendar id="time24" v-model="startDate" :showTime="false" dateFormat="dd.mm.yy" :showSeconds="false" placeholder="Time" />
            </div>
            <label for="endDate">Bitiş</label>
            <div class="field col-12 md:col-4">
              <Calendar id="time24" v-model="endDate" :showTime="false" dateFormat="dd.mm.yy" :showSeconds="false" placeholder="Time" />
            </div>
          </div>
          <client-only>
...
export default {
  ...
  data: () => {
    return {
      ...
      dtColumns: [
        ...
        {
          data: "processDate", title: "Çıkış Tarihi", visible: true,
          render: function (data, ev, row) {
            return row && row.processDate ? moment(row.processDate).format('YYYY.MM.DD HH:mm') : '';
          },
        },
  ...
  methods: {
    ...
    clickDetail: function (e, dt, type, indexes) {
      this.$router.push('/user/checkIn?id=' + this.visualData[indexes[0]].id);
    },
    newRecord() {
