              ...
              <fieldset class="uk-fieldset uk-fieldset-alt md-bg-white sc-padding-medium">
                <div class="uk-child-width-1-2@m uk-grid" data-uk-grid>
                  <div class="uk-width-1-2@m">
                    <ScInput v-model="formData.departmentCode" read-only>
                      <label>Departman Kodu</label>
                    </ScInput>
                  </div>
                  <div class="uk-width-1-2@m">
                    <ScInput v-model="formData.departmentName">
                      <label>Departman Adı</label>
                    </ScInput>
                  </div>
                  <div class="uk-width-1-2@m">
                    <client-only>
                      <Select2 v-model="formData.plantId" :options="plants" single
                        :settings="{ 'width': '100%', 'placeholder': 'İşletme', 'allowClear': true }">
                      </Select2>
                    </client-only>
                  </div>
                </div>
  ...
  export default {
    ...
    methods: {
      async bindModel() {
        ...
        const api = useApi();
        try {
          const getData = (await api.get('Department/' + this.formData.id)).data;
          const plantData = (await api.get('Plant')).data;
          if (plantData)
            this.plants = plantData.map((d) => {
              return {
                ...d,
                text: d.plantName,
              };
      ...
      async onSubmit() {
        try {
          const api = useApi();
          const postResult = (await api.post('Department', this.formData)).data;
      ...
      onCancel() {
        this.$router.push('/user/department/list');
      },
      async onDelete() {
        const self = this;
        UIkit.modal.confirm('Bu departmanı silmek istediğinizden emin misiniz?').then(
          async function () {
            try {
              const api = useApi();
              const delResult = (await api.delete('Department/' + self.formData.id)).data;
              if (delResult.result) {
