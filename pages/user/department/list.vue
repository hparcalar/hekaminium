...
export default {
  ...
  data: () => {
    return {
      visualData: [],
      dtColumns: [
        { data: "departmentCode", title: "Departman Kodu", visible: true, width: "25%" },
        { data: "departmentName", title: "Departman Adı", visible: true, width: "50%" },
        { data: "plantId", title: "Firma Adı", visible: true, width: "25%" },
      ],
      dtDHeaders: [],
      dtDOptions: {
        select: true,
  ...
  async mounted() {
    const api = useApi();
    const rawData = (await api.get('Department')).data;
    const plantData = (await api.get('Plant')).data;

    if(rawData){
      rawData.forEach((item, index) => {
        item.plantId = plantData.find(x => x.id == item.plantId).plantName;
      });
    }
  ...
  methods: {
    ...
    clickDetail: function (e, dt, type, indexes) {
      this.$router.push('/user/department?id=' + this.visualData[indexes[0]].id);
    },
    newRecord() {
